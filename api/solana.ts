...
export async function verifySolanaTransaction(signature: string, mandate: GratitudeMandate, recipient: string): Promise<void> {
  ...
  const rpcResponse = await fetch(process.env.SOLANA_RPC_URL ?? "https://api.devnet.solana.com", {
  ...
  let payload: { result?: { blockTime?: number; meta?: { err?: unknown; preBalances?: number[]; postBalances?: number[] }; transaction?: { signatures?: string[]; message?: { accountKeys?: Array<string | { pubkey?: string }>; instructions?: Array<{program?:string; programId?:string; parsed?: string | {type?: string; info?:{destination?:string; lamports?: number}}}> } } } };
  try {
    payload = await rpcResponse.json() as typeof payload;
  } catch {
    throw new PublicError("PAYMENT_NOT_CONFIRMED", "Solana devnet RPC returned an invalid response.", 502);
  }
  ...
  if (transaction.blockTime == null) throw new PublicError("PAYMENT_NOT_CONFIRMED", "The finalized transaction block time is unavailable.", 409);
  const blockTime = transaction.blockTime * 1000;
